...
if [ -c "${TTYDEV}" ]; then							# check for tty device
  ...
  while true; do								# main loop
    if [ -r ${corenamefile} ]; then						# proceed if file exists and is readable (-r)
      ...
      wget -q 'http://mary.dfki.de:59125/process?INPUT_TEXT=Core started.&INPUT_TYPE=TEXT&OUTPUT_TYPE=AUDIO&AUDIO=WAVE_FILE&LOCALE=en_US&VOICE=cmu-rms' -O core.wav -O - | aplay -q -
      if [ "${debug}" = "false" ]; then
